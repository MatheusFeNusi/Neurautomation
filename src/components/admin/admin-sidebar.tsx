...
export function AdminSidebar({ user }: AdminSidebarProps) {
  ...
  return (
    <aside className="w-64 flex-shrink-0 border-r border-gray-200 bg-white flex flex-col h-screen sticky top-0">
      {/* Brand Header */}
      <div className="h-16 border-b border-gray-200 flex items-center justify-between px-5">
        <Link href="/admin/overview" className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-xs font-bold uppercase tracking-[0.2em] text-gray-900">Neurautomation</div>
            <div className="text-[10px] text-purple-600 font-medium">Affiliate Admin Hub</div>
          </div>
        </Link>
      </div>
      ...
      <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
        <div className="flex items-center justify-between">
          ...
          {user.isDevBypass && (
            <span className="text-[9px] bg-amber-50 border border-amber-200 text-amber-800 px-1.5 py-0.5 rounded">
              DEV MODE
            </span>
          )}
        </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-medium transition-all ${
                isActive
                  ? "bg-gradient-to-r from-purple-100 to-blue-50 text-gray-900 border border-purple-200 font-semibold"
      ...
      <div className="p-3 border-t border-gray-200 bg-white">
        <form action="/auth/signout" method="post">
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-gray-200 text-xs font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-colors"
          >
