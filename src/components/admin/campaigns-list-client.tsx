      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-3 rounded-xl border border-gray-200">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <select
            ...
            className="bg-gray-100 border border-gray-200 rounded-lg px-2.5 py-1 text-xs text-gray-900 focus:outline-none focus:border-blue-500"
          >
                  ...
                  return (
                    <tr key={camp.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3.5 px-3">
                        <div className="font-medium text-gray-800">{store?.name || "Loja Geral"}</div>
                        <div className="text-[10px] text-gray-400">{offer?.name || "Oferta Geral"}</div>
                      </td>
                      <td className="py-3.5 px-3 font-mono text-gray-700">
                        {camp.external_campaign_id ? (
                          <span className="bg-gray-100 px-2 py-0.5 rounded text-[11px]">{camp.external_campaign_id}</span>
                        ) : (
                          <span className="text-gray-400 text-[10px]">Aguardando Sync</span>
                        )}
                      </td>
                      <td className="py-3.5 px-3">
                      ...
                      <td className="py-3.5 px-3 text-right font-medium text-gray-900">
                        ...
                        <div className="text-[10px] text-gray-400">{m.clicks} cliques</div>
                      </td>
                      <td className="py-3.5 px-3 text-right font-semibold text-purple-700">
                        {formatCurrency(m.total_commission)}
                        <div className="text-[10px] text-gray-400">{m.sales_count} vendas</div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white border border-gray-200 rounded-xl w-full max-w-lg p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 pb-3 mb-4">
              <h2 className="text-base font-semibold text-gray-900">Cadastrar Campanha</h2>
            ...
            <form onSubmit={handleCreateCampaign} className="space-y-3.5">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Oferta (Opcional)</label>
                  <select
                    value={offerId}
                    onChange={(e) => setOfferId(e.target.value)}
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-blue-500"
                  >
                    <option value="" className="bg-white text-gray-900">-- Geral --</option>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Fonte de Tráfego</label>
                  <select
                    value={trafficSource}
                  ...
                  >
                    ...
                    <option value="meta_ads" className="bg-white text-gray-900">Meta Ads (Facebook/Insta)</option>
                    <option value="bing_ads" className="bg-white text-gray-900">Microsoft Bing Ads</option>
                    <option value="tiktok_ads" className="bg-white text-gray-900">TikTok Ads</option>
                    <option value="other" className="bg-white text-gray-900">Outra Fonte</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">ID Externo (Google Ads)</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Grupo de Anúncios</label>
                  <input
                    type="text"
                    value={adGroup}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1">Termo Principal de Busca</label>
                  <input
                    type="text"
                    value={searchTerm}
