          ...
          <div className="bg-white border border-gray-200 rounded-xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 pb-4 mb-5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h2 className="text-base font-semibold text-gray-900">Registrar Venda de Afiliado</h2>
                  <p className="text-xs text-gray-500">Lançamento manual com validação de rastreamento e receita</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Loja */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Loja Afiliada *</label>
                  <select
                    value={storeId}
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-500 placeholder:text-gray-400"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Oferta */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Oferta / Produto (Opcional)</label>
                  <select
                    value={offerId}
                    onChange={(e) => setOfferId(e.target.value)}
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-500"
                  >
                    <option value="" className="bg-white text-gray-900">-- Nenhuma oferta específica --</option>
                ...
                {/* Campanha */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Campanha Associada (Opcional)</label>
                  <select
                    value={campaignId}
                    onChange={(e) => setCampaignId(e.target.value)}
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-500"
                  >
                    <option value="" className="bg-white text-gray-900">-- Nenhuma campanha associada --</option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {/* Data */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Data da Venda *</label>
                  <input
                    type="date"
                    value={date}
                ...
                {/* ID do Pedido */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Order ID (Opcional)</label>
                  <input
                    type="text"
                    value={orderId}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-500"
                  >
                    <option value="approved" className="bg-white text-gray-900">Aprovada (Approved)</option>
                    <option value="pending" className="bg-white text-gray-900">Pendente (Pending)</option>
                    <option value="rejected" className="bg-white text-gray-900">Rejeitada (Rejected)</option>
                    <option value="refunded" className="bg-white text-gray-900">Reembolsada (Refunded)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Valor da Venda (R$) *</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                {/* Comissão */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Comissão Gerada (R$) *</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                {/* Moeda */}
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1.5">Moeda</label>
                  <select
                    value={currency}
                    onChange={(e) => setCurrency(e.target.value)}
                    className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-500"
                  >
                    <option value="BRL" className="bg-white text-gray-900">BRL (R$)</option>
              ...
              <div className="p-3.5 rounded-lg border border-purple-200 bg-purple-50/60 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-purple-700">Rastreamento & Origem de Tráfego</span>
                  <span className="text-[10px] text-gray-400">Regra: Click ID obrigatório p/ atribuição direta</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[11px] font-medium text-gray-700 mb-1">Origem do Tráfego *</label>
                    <select
                      value={origin}
                      onChange={(e) => setOrigin(e.target.value as SaleOrigin)}
                      className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-400"
                    >
                      <option value="manual" className="bg-white text-gray-900">Manual / Não rastreada</option>
                      <option value="google_ads" className="bg-white text-gray-900">Google Ads (exige Click ID ou confirmação)</option>
                      <option value="organic" className="bg-white text-gray-900">Orgânico (SEO)</option>
                      <option value="direct" className="bg-white text-gray-900">Direto</option>
                      <option value="social" className="bg-white text-gray-900">Redes Sociais</option>
                      <option value="email" className="bg-white text-gray-900">E-mail Marketing</option>
                      <option value="unknown" className="bg-white text-gray-900">Desconhecido</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-[11px] font-medium text-gray-700 mb-1">Click ID (gclid / affiliate click)</label>
                    <input
                      type="text"
                      value={clickId}
                      onChange={(e) => setClickId(e.target.value)}
                      placeholder="Ex: gclid_EAIaIQobChM..."
                      className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-xs text-gray-900 focus:outline-none focus:border-purple-400 placeholder:text-gray-400"
                    />
                  </div>
                </div>
