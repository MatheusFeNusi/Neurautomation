    ...
    <div className="space-y-6">
      <div className="border-b border-gray-200 pb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-gray-100 border border-gray-200 text-gray-700 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed self-start"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="text-[11px] text-gray-400 mt-1">Disponíveis para resgate / confirmadas</div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="text-[11px] text-gray-400 mt-1">Aguardando janela de estorno da rede</div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="text-[11px] text-gray-400 mt-1">Cancelamentos ou devoluções no e-commerce</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-gray-200 pb-3">
            ...
            <span className="text-xs text-gray-400">{networkBreakdown.length} redes ativas</span>
          </div>

          <div className="space-y-3">
            {networkBreakdown.length === 0 ? (
              <div className="py-8 text-center text-gray-400 text-xs">Nenhuma venda conciliada ainda.</div>
            ) : (
              networkBreakdown.map((net) => (
                <div key={net.network} className="p-3 bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-between">
                  <div>
                    <div className="font-semibold text-gray-900 text-xs">{net.network}</div>
                    <div className="text-[11px] text-gray-400 mt-0.5">{net.salesCount} vendas geradas</div>
                  </div>

                  <div className="text-right">
                    <div className="text-xs font-bold text-purple-700">{formatCurrency(net.total)}</div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-gray-200 pb-3">
            ...
            <span className="text-xs text-gray-400">Ranking por faturamento</span>
          </div>

          <div className="space-y-3">
            {storeBreakdown.length === 0 ? (
              <div className="py-8 text-center text-gray-400 text-xs">Nenhum dado cadastrado.</div>
            ) : (
              storeBreakdown.slice(0, 5).map((st) => (
                <div key={st.storeName} className="p-3 bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-between">
                  <div>
                    <div className="font-semibold text-gray-900 text-xs">{st.storeName}</div>
                    <div className="text-[11px] text-gray-400 mt-0.5">{st.network} • {st.salesCount} vendas</div>
                  </div>

                  <div className="text-right">
                    <div className="text-xs font-bold text-gray-900">{formatCurrency(st.total)}</div>
